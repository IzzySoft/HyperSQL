/**
 * Extract view definitions from data dictionary
...
DECLARE
  ...
  CURSOR c_grant(tname IN VARCHAR2) IS
    SELECT (CASE WHEN grantable='NO' THEN 'GRANT '||privilege||' ON '||tname||' TO '||grantee||';'
      ...
      FROM user_tab_privs
     WHERE table_name='EOX_ARBEITSTAGE';
BEGIN
  ...
  FOR rec IN c_tab LOOP
    ...
    END LOOP;
    utl_file.putf(fh,' * @view '||rec.table_name||'\n');
    FOR r1 IN c_colcomment(rec.table_name) LOOP
      utl_file.put_line(fh,r1.line);
    END LOOP;
    utl_file.put_line(fh,' */');
    utl_file.putf(fh,dbms_metadata.get_ddl( object_type => 'VIEW' , name => rec.table_name)||'\n');
    BEGIN
