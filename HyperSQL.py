...
def MakeHTMLFooter():
    ...
    s += "<DIV ID='bottombar'>\n"
    s += MakeNavBar()
    s += "  <DIV ID='generated'>Generated by <A HREF='http://hypersql.sourceforge.net/'>HyperSQL</A> v" + metaInfo.versionString + " at " + time.asctime(time.localtime(time.time())) + "</DIV>";
    s += "</DIV>\n"
    s += "</body></html>"
    return s
...
def CreateIndexPage(meta_info):
    ...
    if os.path.exists(meta_info.css_file):
      scriptinfile  = open(meta_info.css_file,'r')
      scriptoutfile = open(html_dir + os.path.split(meta_info.css_file)[1], 'w');
      scriptoutfile.write(scriptinfile.read())
      scriptoutfile.close()
      scriptinfile.close()

    outfile.write("<CENTER><B>" + metaInfo.title_prefix + " HyperSQL Reference</B></CENTER><BR><BR>\n")
    ...
    outfile.write("<BR><BR>\n")
    outfile.write("<TABLE WIDTH='600px' ALIGN='center'><TR><TD>\n")
